/**
 * DB — 場景互動與造訪記錄
 * 包含：scene_interactions、scene_visits
 */
...
export async function getSceneInteractions(
  ...
  try {
    ...
    if (error) {
      console.warn("[db-scene] getSceneInteractions:", error.message);
      return [];
    }
    return (data ?? []) as SceneInteraction[];
  } catch (e) {
    console.warn("[db-scene] getSceneInteractions:", e);
    return [];
  }
}
...
export async function recordSceneInteraction(
  ...
  try {
    ...
    console.warn("[db-scene] recordSceneInteraction:", e);
  }
}
...
export async function getSceneVisit(
  ...
  try {
    ...
    const { data, error } = await db
      ...
      .maybeSingle();
    if (error) {
      console.warn("[db-scene] getSceneVisit:", error.message);
      return null;
    }
    return data as SceneVisit | null;
  } catch (e) {
    console.warn("[db-scene] getSceneVisit:", e);
    return null;
  }
}
...
export async function recordSceneVisit(
  ...
  try {
    ...
    console.warn("[db-scene] recordSceneVisit:", e);
  }
}
